...
def pansharpening(img, pan_img, algorithm):
    ...
    if algorithm == 'dwt':
        ...
        I_mean = np.mean(I)
        P = ((pan_img - pan_mean) * (np.std(I) / np.std(pan_img))) + I_mean
        pansharpened_img = np.zeros(img.shape)
        for band in range(d):
            pansharpened_img[:, :, band] = img[:, :, band] + P - I
        return pansharpened_img.astype('uint8')
    elif algorithm == 'sfim':
        rows, cols, d = img.shape
        r = 2
...
class results:
    ...
    def table_results(self):
        ...
        dic['CC'] = self.cc(); dic['SSIM'] = self.ssim() ; dic['ERGAS'] = self.ergas()
        dic['UIQI'] = self.uiqi()
    ...
    def uiqi(self):
        ...
        mean_ms = np.mean(self.img)
        mean_ref = np.mean(self.ref)
        std_ms = np.std(self.img)
        std_ref = np.std(self.ref)
        covariance = np.sum((self.img - mean_ms) * (self.ref - mean_ref)) / (rows * cols - 1)
        pay = 4 * mean_ms * mean_ref * covariance
        payda = (mean_ms ** 2 + mean_ref ** 2) * (std_ms ** 2 + std_ref ** 2)
        return pay / payda
